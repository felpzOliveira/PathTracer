...
int box_z_compare (const void * a, const void * b, void * arg) {
    ...
    get_aabb(scene, *ah, &box_left);
    get_aabb(scene, *bh, &box_right);
...
inline __host__ BVHNode *build_bvh(Scene *scene, Object *handles, 
                                   int n, int depth, int max_depth)
{
    ...
    if (axis == 0)
        qsort_r(handles, n, sizeof(Object), box_x_compare, scene);
    else if (axis == 1)
        qsort_r(handles, n, sizeof(Object), box_y_compare, scene);
    else
        qsort_r(handles, n, sizeof(Object), box_z_compare, scene);
    
    if(n == 1){
        bvh_node_set_itens(node, 1);
        memcpy(node->handles, handles, sizeof(Object)*n);
        get_aabb(scene, handles[0], &node->box);
        node->is_leaf = 1;
        return node;
        ...
        get_aabb(scene, handles[0], &node->left->box);
        get_aabb(scene, handles[1], &node->right->box);
        ...
        for(int i = 1; i < n; i += 1){
            AABB aabb;
            get_aabb(scene, handles[i], &aabb);
            surrounding_box(&node->box, &aabb, &node->box);
        }
        node->is_leaf = 1;
        return node;
    }else{
        node->left = build_bvh(scene, handles, n/2, depth+1, max_depth);
        node->right = build_bvh(scene, &handles[n/2], n-n/2, depth+1, max_depth);
    }

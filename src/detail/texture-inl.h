...
#error "Please include texture.h instead of this file"
#else

inline __bidevice__
glm::vec3 texture_value_no_recursion(Texture *texture, float u, 
                                     float v, glm::vec3 &p, Scene *scene);

inline __bidevice__
glm::vec3 texture_value_const(Texture *texture, float u, float v, 
                              glm::vec3 &p, Scene *scene)
{
    return texture->color;
}

inline __bidevice__
glm::vec3 texture_value_checker(Texture *texture, float u, float v,
                                glm::vec3 &p, Scene *scene)
{
...
inline __bidevice__
glm::vec3 texture_value_noise(Texture *texture, float u, float v,
                              glm::vec3 &p, Scene *scene)
{
...
inline __bidevice__
glm::vec2 texture_apply_distribution(Scene *scene, float u, float v,
                                     Texture *texture)
{
...
inline __bidevice__
glm::vec3 texture_value_image(Texture *texture, float u, float v,
                              glm::vec3 &p, Scene *scene)
{
...
Spectrum texture_value(Texture *texture, hit_record *record, Scene *scene){
    ...
    switch(texture->type){
        case TEXTURE_CONST: {
            val = texture_value_const(texture, u, v, p, scene); 
        } break;
        case TEXTURE_NOISE: {
            val = texture_value_noise(texture, u, v, p, scene);
        } break;
        case TEXTURE_IMAGE: {
            val = texture_value_image(texture, u, v, p, scene);
        } break;
        case TEXTURE_CHECKER: {
            val = texture_value_checker(texture, u, v, p, scene);
        } break;
        
        default: val = glm::vec3(0.0f);
    }
    
    return Spectrum::FromRGB(val.x, val.y, val.z);
}

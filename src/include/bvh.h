...
inline __host__ 
void bvh_node_mark_itens(BVHNode *node, int n){
    if(node){
        if(n > 0){
            node->n_handles = n;
        }
...
inline __host__ 
void bvh_node_set_itens(BVHNode *node, int n){
    if(node){
        if(n > 0){
...
inline __host__ 
int get_bvh_node_count(BVHNode *root){
    if(root){
        if(root->is_leaf){
            return 1;
        }
...
template<typename Q>
inline __host__ 
BVHNode *_build_bvh(Q *locator, Object *handles, 
                   int n, int depth, int max_depth, int *totalNodes)
{
    ...
    if(n == 1){
        ...
        node->left = _build_bvh<Q>(locator, handles, n/2, depth+1, max_depth, totalNodes);
        node->right = _build_bvh<Q>(locator, &handles[n/2], n-n/2, depth+1, max_depth, totalNodes);
    }
...
template<typename Q>
inline __host__ 
BVHNode *build_bvh(Q *locator, Object *handles, 
                   int n, int depth, int max_depth)
{
    int totalNodes = 0;
    BVHNode *node = _build_bvh<Q>(locator, handles, n, depth, max_depth, &totalNodes);
    std::cout << "Built BVH with " << totalNodes << " nodes" << std::endl;
    return node;
}
